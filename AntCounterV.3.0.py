...
class AntCounter:
            ...
            for j in range (len(matrizIndices[frameNo-2])): #anterior
                if matrizIndices[frameNo-1][i]== matrizIndices[frameNo-2][j]:
                    ...
                    if matrizIndices[frameNo-1][i] in counted:
                        pass
                    elif (self.height/2) - 10 < y1 < (self.height/2) and (self.height/2)+10 > y2 >= (self.height/2) :
                        antsUp +=1
    ...
    def run(self):
        ...
        for i in range ((len(self.videoFileNameList))):
            ...
            fps =      long(cv.GetCaptureProperty(self.capture, cv.CV_CAP_PROP_FPS))
            if self.duration*fps > longvideo:
                ...
                while True:
                    ...
                    cv.Rectangle(color_image,(0,0),(self.width,(self.height/4)),(0,0,0),cv.CV_FILLED,1)
                    cv.Rectangle(color_image,(0,(self.height/4*3)),(self.width,self.height),(0,0,0),cv.CV_FILLED,1)

                    #trace the lines and show tracking zone
                    cv.Line(original_image,(0,(self.height/4)),(self.width,(self.height/4)),(255,255,255),2,8,0)
                    cv.Line(original_image,(0,(self.height/4*3)),(self.width,(self.height/4*3)),(255,255,255),2,8,0)

                    #trace counting line
                    cv.Line(original_image,(0,self.height/2),(self.width,self.height/2),(0,255,0),3,8,0)
                    ...
                    if len(self.videoFileNameList) == 1:
                        if frameNumero == self.duration*fps:
                            self.graficar()
                            self.saveFile()
                            break
                    else:
...
class App:
    ...
    def okButtonCommand(self):
        ...
        if self.validation():
            self.duration = ((int(self.strMin.get())*60)+(int(self.strSeg.get())))
